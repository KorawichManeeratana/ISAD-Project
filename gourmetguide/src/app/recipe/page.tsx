...
export default class page extends Component<{ searchParams: any }> {
  ...
  async componentDidUpdate(prevProps: Readonly<{ searchParams: any; }>) {
      if (this.props.searchParams.searchResult !== prevProps.searchParams.searchResult){
        this.handleSearch()
      }
  }
  ...
  render() {
    return (
      <div className="bg-gray-200">
        ...
        <div className="w-[90vw]  h-auto m-auto border-20 pt-10 border-x-gray-400 bg-yellow-100 px-8">
        <div className="flex items-center justify-center mt-8">
              ...
              <button
                 onClick={() => this.handleSearch() }className="bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-2 px-6 ml-10 rounded-l-3xl rounded-r-3xl"
              ><Link  href={{
                pathname: '/recipe',
                query: {
            ...
            <div className="mt-4">
              <div className="flex justify-start items-start pt-10 pl-6"><h1 className="text-2xl text-gray-500">ผลการค้นหา "{this.props.searchParams.searchResult}"</h1></div>
              <div className="flex justify-end items-end">
                <button className="bg-white hover:bg-yellow-600 text-yellow-800 font-medium py-2 px-6  rounded-l-3xl rounded-r-3xl">ค้นหาแบบละเอียด</button>
                <button className="bg-white hover:bg-yellow-600 text-yellow-800 font-medium py-2 px-6 ml-8 mr-28 rounded-l-3xl rounded-r-3xl">ค้นหาแบบละเอียด</button>
              </div>
            </div>
          {this.state.items &&

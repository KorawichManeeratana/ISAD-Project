...
export default class loginSystem {
    public static async checkLogin(email: string, password: string): Promise<boolean> {
        ...
        try {
            const result : any = await qb.selectFrom("account").select(["account.ac_id", "account.password", "account.role", "account.userPFP", "account.username"]).where("account.email", "=", email).execute();
            console.log("rowsInCheck:", result[0])
            console.log("password:", result[0].password)
            console.log("id:", result[0].ac_id)
            console.log("role:", result[0].role)
            if (bcrypt.compare(password, result[0].password)) {
                let output: {access: string, refresh: string} = await tokenManage.getNewToken(result[0].ac_id, result[0].role, result[0].userPFP, result[0].username);
